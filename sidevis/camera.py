### Modified panzoom camera, inspired by https://raw.githubusercontent.com/vispy/vispy/main/vispy/scene/cameras/panzoom.py
...
class ScaZoomCamera(BaseCamera):
    ...
    **kwargs : dict
    ...
    def __init__(self, rect=DEFAULT_RECT_TUPLE, aspect=None, **kwargs):
        super(ScaZoomCamera, self).__init__(**kwargs)
        ...
        if event.type == 'mouse_wheel':
            ...
            if 1 in event.buttons:
                if not modifiers:
                    ...
                    # Update the underlying x data instead of the camera
                    line = next(filter(lambda x: isinstance(x, Line), event.source.scene.children))
                    data = line.pos.copy()
                    p1 = self.x_align_start
                    p2 = np.array(event.pos)[:2]
                    pan = self._transform.imap(p2) - self._transform.imap(p1)
                    x_shift = pan[0]
                    data.T[0] = self.x_align_orig + x_shift
                    line.set_data(data)
                    event.handled = True
            ...
            elif 1 in event.buttons and len(modifiers) == 1 and modifiers[0].name == keys.CONTROL:
                # Record start point of clicking control
                self.x_align_start = np.array(event.last_event.pos)[:2]
                line = next(filter(lambda x: isinstance(x, Line), event.source.scene.children))
                self.x_align_orig = line.pos.T[0].copy()
                # print(f'press {self.x_align_start=}')
                event.handled = True
            else:
                pass
        elif event.type == 'mouse_release':
            # if len(modifiers) == 1 and modifiers[0].name == keys.CONTROL:
            #     print(f'done aligning {event.mouse_event.pos - self.align_start}')
